        ...
        if (!gamePad) {
            ...
            if (gamePad) {
                gamePad->RegisterStatusHandler([](unsigned index, const TGamePadState* state) {
                    msxPad1 = 0;
                    msxPad1 |= state->axes[0].value == state->axes[0].minimum ? MSX1_JOY_LE : 0;
                    msxPad1 |= state->axes[0].value == state->axes[0].maximum ? MSX1_JOY_RI : 0;
                    msxPad1 |= state->axes[1].value == state->axes[1].minimum ? MSX1_JOY_UP : 0;
                    msxPad1 |= state->axes[1].value == state->axes[1].maximum ? MSX1_JOY_DW : 0;
                    msxPad1 |= (state->buttons & 0x0001) ? MSX1_JOY_T2 : 0;
                    msxPad1 |= (state->buttons & 0x0002) ? MSX1_JOY_T1 : 0;
                    msxPad1 |= (state->buttons & 0x0100) ? MSX1_JOY_S2 : 0;
                    msxPad1 |= (state->buttons & 0x0200) ? MSX1_JOY_S1 : 0;
                });
            }
        } else {
